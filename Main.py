...
def trainNetwork():
    ...
    for i in range(num_runs):
        nn.train(train_imgs, train_labels_one_hot)
        print('Finished run %2d', i)
    ...
    weights_matrices = nn.weights_matrices
    storeWeights(weights_matrices)
    
def storeWeights(weights_matrices):
    with open(data_path + "pickled_weights.pkl", "bw") as fh:
        data = weights_matrices
        pickle.dump(data, fh)
    
#@timer 
def singleResult(arr):
